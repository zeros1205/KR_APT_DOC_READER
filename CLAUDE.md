...
## 프로젝트 개요
청약홈 분양공고 데이터를 수집·분석하여 정과장의 청약노트(`https://apt-note.com`)에 게시할 HTML 페이지를 자동 생성하는 파이프라인.
배포 방식: 정적 HTML 사이트(Cloudflare Pages 등). `<script>`·외부 CSS 모두 허용. 디자인 일관성을 위해 인라인 스타일을 권장하지만 강제는 아님.
네이버 블로그 발행은 더 이상 타깃이 아니다. 과거 `templates/blog_template.html`이 body fragment로 남아 있는 것은 `pipeline/html_renderer.py`가 `<!DOCTYPE>`/`<head>`/`<body>`를 동적으로 감싸 단일 정적 페이지를 만들어내기 때문이며, 그 자체로 발행 대상이 아니다.
