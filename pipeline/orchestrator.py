...
- eligibility_special: 배열 — 공고에 명시된 특별공급 유형별 신청자격
  각 항목: type_name(예: "생애최초", "신혼부부", "다자녀", "신생아", "노부모부양", "기관추천", "지역균형발전", "청년"),
           quota(예: "20호" — 없으면 null),
           requirements(핵심 자격 요건 문장 목록, 최대 4개, 공고 원문 기준)
  ※ 2026-06-15 주택공급규칙 개정으로 "신생아 특별공급"(별도 물량 약 10%)과 "지역균형발전 특별공급"이 신설됐으니,
...
def _special_supply_fallback_items(base_items: list[dict]) -> list[dict]:
    fallback_rules = {
        ...
        "지역균형발전": [
            "지역균형발전 특별공급은 지자체가 정한 공급 대상(이주자·이전기업 종사자 등)과 거주요건을 공고문 기준으로 확인해야 합니다.",
            "무주택 여부와 청약통장 요건은 모집공고문 및 청약홈 안내를 함께 확인해야 합니다.",
        ],
    }
    result = []
...
def _special_supply_standard_requirements_2026(type_name: str) -> list[str]:
    rules = {
        ...
        "지역균형발전": [
            "지역경제 활성화·균형발전 시책에 따라 지방자치단체가 정한 대상(지역 이주자, 이전기업 종사자 등)에게 공급하는 2026년 신설 유형입니다.",
            "공급 대상·거주요건·무주택 여부·청약통장 조건은 해당 지자체 고시와 모집공고문 기준으로 반드시 확인해야 합니다.",
        ],
        }
    return rules.get(
        type_name,
