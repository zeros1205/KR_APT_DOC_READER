...
- eligibility_special: 배열 — 공고에 명시된 특별공급 유형별 신청자격
  각 항목: type_name(예: "신생아", "신혼부부", "생애최초", "다자녀", "노부모부양", "기관추천"),
           quota(예: "20호" — 없으면 null),
           requirements(핵심 자격 요건 문장 목록, 최대 4개, 공고 원문 기준)
  ※ 2026-06-15 개정으로 신설된 "신생아 특별공급"(별도 물량 약 10%)은 공고에 있으면 반드시 별도 유형으로 추출하세요.
  ※ 기관추천의 세분류(국가유공자·장애인·중소기업·군인·지방 이전기업 등)는 개별로 나열하지 말고 "기관추천" 하나로 묶어 추출하세요.
...
def _canonical_supply_type(type_name: str) -> str:
    """공고문 특별공급 유형명을 우리 페이지의 표준 유형으로 정규화한다.

    기관추천의 세분류(국가유공자·장애인·중소기업 장기근속·군인·지방 이전기업 등)는
    개별로 나열하지 않고 "기관추천" 하나로 묶는다.
    """
    n = (type_name or "").replace(" ", "")
    if "신생아" in n:
        return "신생아"
    if "신혼" in n:
        return "신혼부부"
    ...
    if "노부모" in n:
        return "노부모부양"
    institutional = (
        "기관추천", "국가유공", "유공자", "보훈", "장애인", "중소기업", "장기근속",
        "군인", "제대", "이전기업", "지역균형", "지역발전", "북한이탈", "다문화",
        "기능인", "체육",
    )
    if any(k in n for k in institutional):
        return "기관추천"
    return (type_name or "").strip()


def _special_supply_standard_requirements_2026(type_name: str) -> list[str]:
    """유형별 핵심 3가지(대상 / 자격·소득요건 / 당첨자 선정방식)만 제시한다.

    소득·자산 수치는 최신 정책의 표준값을 안내하되, 공공·민영 및 공고별로
    달라질 수 있는 부분은 공고문 확인으로 위임한다.
    """
    canon = _canonical_supply_type(type_name)
    rules = {
        "신생아": [
            "대상: 만 2세 미만(태아·입양아 포함) 자녀가 있는 무주택세대구성원으로, 혼인 여부·혼인기간과 무관하게 신청할 수 있습니다.",
            "자격·소득: 소득 130% 이하(맞벌이 200% 이하)가 기본이며, 자산·청약통장 요건은 공고문 기준으로 확인해야 합니다.",
            "선정: 소득 구간별 우선·일반 배정 후 추첨으로 당첨자를 정합니다.",
        ],
        "신혼부부": [
            "대상: 혼인 7년 이내 무주택세대구성원으로, 예비신혼부부 포함 여부는 공고문에서 확인해야 합니다.",
            "자격·소득: 소득 100% 이하 우선공급이 기본이며, 맞벌이·자산·청약통장 요건은 공고문 기준으로 확인해야 합니다.",
            "선정: 자녀가 있는 세대를 우선하고, 미성년 자녀 수가 많은 순으로 당첨자를 정합니다.",
        ],
        "생애최초": [
            "대상: 세대구성원 전원이 과거 주택을 소유한 적 없는 무주택세대구성원입니다.",
            "자격·소득: 기혼이거나 자녀가 있어야 하며(1인가구는 추첨 대상), 5년 이상 소득세 납부·소득·자산 요건은 공고문에서 확인해야 합니다.",
            "선정: 자격을 갖춘 신청자 중 100% 추첨으로 당첨자를 정합니다.",
        ],
        "다자녀가구": [
            "대상: 미성년 자녀 2명 이상(태아 포함)인 무주택세대구성원입니다.",
            "자격·소득: 소득 120% 이하가 기본이나 공공·민영 공고별로 다르므로 공고문에서 확인해야 합니다.",
            "선정: 다자녀 배점표 고득점순으로 당첨자를 정합니다.",
        ],
        "노부모부양": [
            "대상: 만 65세 이상 직계존속을 3년 이상 계속 부양한 무주택세대구성원으로, 반드시 세대주만 신청할 수 있습니다.",
            "자격·소득: 부양 중인 직계존속도 무주택이어야 하며, 소득·자산·청약통장 요건은 공고문에서 확인해야 합니다.",
            "선정: 일반공급 가점제 고득점순으로 당첨자를 정합니다.",
        ],
        "기관추천": [
            "대상: 국가유공자·장애인·중소기업 장기근속자·군인·지방 이전기업 종사자 등 여러 세분류를 아우르며, 소관 기관장의 추천 대상자로 선정되는 것이 핵심입니다.",
            "자격·소득: 세분류별 요건이 다르며(국가유공자·장애인은 청약통장 없이도 가능), 무주택·소득·청약통장 요건은 공고문과 기관 안내에서 확인해야 합니다.",
            "선정: 기관 자체 심사·추천을 거쳐 배정하며, 세부 방식은 공고문 기준으로 확인해야 합니다.",
        ],
    }
    return rules.get(
        canon,
        [
            "대상: 입주자모집공고일 기준 무주택세대구성원 여부와 해당 유형의 기본 자격을 먼저 확인해야 합니다.",
            "자격·소득: 혼인·자녀·연령·부양기간·소득·자산·청약통장 조건을 공고문에서 확인해야 합니다.",
            "선정: 당첨자 선정방식은 공고문 기준으로 확인해야 합니다.",
        ],
    )
...
def _apply_special_supply_2026_guidance(facts: dict) -> dict:
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        raw = str(item.get("type_name") or "").strip()
        if not raw:
            continue
        canon = _canonical_supply_type(raw)
        if canon in seen:
            # 기관추천 세분류 등 중복 유형은 하나로 묶고, quota만 보완한다.
            idx = seen[canon]
            if not normalized[idx].get("quota") and item.get("quota"):
                normalized[idx]["quota"] = item.get("quota")
            continue
        seen[canon] = len(normalized)
        normalized.append(
            {
                "type_name": canon,
                "quota": item.get("quota"),
                "requirements": _special_supply_standard_requirements_2026(canon),
            }
        )
    if normalized:
